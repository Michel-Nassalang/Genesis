...
import 'flick_video/appinio_video.dart';
...
//-----------------------
  ...
  Future<void> loadimg() async {
    ...
    await dio.download(
      ...
      '$dirdoc/${widget.message.timestamp}.jpeg',
  ...
  Future<void> openFile(Message sms) async {
    String dirdoc = await _localPath;
    String path = '$dirdoc/${sms.name}';
  ...
  String extension(String nom) {
    final part = nom.split(".");
    String ext = part.last;
    return ext;
  }

  Future<PDFDocument> get fichier async {
    String dirdoc = await _localPath;
    String path = '$dirdoc/${widget.message.name}';
    if (!io.File(path).existsSync()) {
      PDFDocument fichier = await PDFDocument.fromURL(widget.message.content);
    } else {
      File file = File(path);
      PDFDocument fichier = await PDFDocument.fromFile(file);
    }
    return fichier;
  }
              ...
              child: (widget.message.type == 1)
                  ...
                  : (widget.message.type == 2)
                      ...
                      GestureDetector(
                          ...
                          child: io.File(
                                      '$_localPath/${widget.message.timestamp}.jpeg')
                                  .existsSync()
                              ? Image.file(
                                  File(
                                      '$_localPath/${widget.message.timestamp}.jpeg'),
                      ...
                      : (widget.message.type == 3)
                          ...
                          : (widget.message.type == 4)
                              ...
                              GestureDetector(
                                  ...
                                  child: extension(widget.message.name) != "pdf"
                                      ? Row(
                                      ...
                                      : SizedBox(
                                          height: 150,
                                          width: 150,
                                          // child : PDFViewer(document: fichier)
                                          child: FutureBuilder<PDFDocument>(
                                            future: fichier,
                                            builder: (BuildContext context,
                                                AsyncSnapshot<PDFDocument>
                                                    snapshot) {
                                              switch (
